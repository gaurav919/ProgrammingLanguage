...
public class CFConditional extends CFExp {
	...
	public String toString() {
		...
		return "( " + leftTest.toString() + op + rightTest.toString() + " ? " + trueChoice.toString() + " : "
				+ falseChoice.toString() + " )";
	}
	 ...
	 * YOU MUST CODE THIS
	 * 
	 * if env is null throw an Exception if env with message
	 * "error in eval : env is null"
	 * 
	 * if there is a variable in the expression tree with no entry in env throw an
	 ...
	 * 
	 * Otherwise, return the value of the expression obtained by using the values
	 * given in env
	 * 
	 ...
	 * leftTest rightTest
	 * 
	 * then perform the comparison indicated by relop and ONLY evaluate whichever of
	 * the trueChoice and falseChoice subexpressions the boolean result selects. So,
	 * for example
	 * 
	 ...
	 * 
	 * should neither of them throw an exception for x being not bound to a value
	 * 
	 * 
	 **********************************************************************/
	...
	public CofinFin eval(Map<String, CofinFin> env) throws Exception {
		...
		if (relop == CFToken.EQUALS) {
			if (leftTest.eval(env).equals(rightTest.eval(env))) {
				return trueChoice.eval(env);
			} else {
				return falseChoice.eval(env);
			}
		}
		if (relop == CFToken.SUBSETOF) {
			if (leftTest.eval(env).isSubsetOf(rightTest.eval(env))) {
				return falseChoice.eval(env);
			} else {
				return trueChoice.eval(env);
			}
		} else {
			throw new Exception("error in eval : variable " + relop + " is not bound in the environment");
		}
	 ...
	 * 
	 * otherwise return a CFConditional object that is the result of applying the
	 * substitutions of bindings to this object.
	 * 
	 * Because the old value is not needed, it is adequate to replace the four
	 * subexpressions with their substitute results and return this.
	 * 
	 **********************************************************************/
	...
	public CFExp substitute(Map<String, CFExp> bindings) throws Exception {
		if(bindings == null) {
			...
			return new CFConditional(relop, bindings.get("leftTest"),bindings.get("rightTest"),bindings.get("trueChoice"),bindings.get("falseChoice"));
		}
